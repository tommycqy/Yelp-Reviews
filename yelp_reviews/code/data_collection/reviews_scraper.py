...
def extract_reviews(url):
    ...
    Returns: newest_page_reviews (list): list of reviews on the latest page (max 20)
    """
    api_url = url + "%3Fstart%3D40"
    html_obj = retrieve_html(url)[1]
    newest_page_reviews = parse_page(html_obj)
    return newest_page_reviews
...
def write_data():
    ...
    with open('reviewCountOnPage.csv', 'w') as f:
        writer = csv.writer(f) 
